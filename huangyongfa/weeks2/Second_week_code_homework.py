...
5.训练好的模型输出（训练好，指得到了对应理想的权重）
 ...
 任务目标:
    ...
    任务1 随机创建一个单位长度为5的向量，如果向量中所有奇数位置的元素之和大于所有偶数位置元素之和，则将该向量标记为正样本（1），反之则标记为负样本（0）。
    
    任务2 随机创建一个单位长度为10的向量，如果向量中所有质数位置的元素之和（位置1, 2, 3, 5, 7）大于所有非质数位置元素的平均值乘以向量长度（即10），则将该向量标记为正样本（1）；否则，标记为负样本（0）
"""
# 1.数据预处理
def build_sample():
    """
